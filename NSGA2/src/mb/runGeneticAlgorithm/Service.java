...
public class Service {
     ...
     */
    public static HashMap<Integer, List<Query>> fastNonDominatedSort(Population population) {
        
        HashMap<Integer, List<Query>> paretoFront = new HashMap<>();
        List<Query> singularFront = new ArrayList<>();
        List<Query> populace = population.getPopulation();
        
        /**
         * iterating over each chromosome of the population
         */
        for(Query chromosome : populace) {
             ...
             */
            for (Query competitor : populace) if(!competitor.equals(chromosome)) {
        ...
        int i = 1;
        List<Query> previousFront = paretoFront.get(i);
        List<Query> nextFront = new ArrayList<>();
        ...
        while(previousFront != null && !previousFront.isEmpty()) {
             ...
             */
            for(Query chromosome : previousFront) {
                
                /**
                 * iterating over each of the dominated chromosomes from the present chromosome of rank i.
                 */
                for(Query recessive : chromosome.getDominatedChromosomes()) {
            ...
            if(nextFront.isEmpty() && !isDominatedChromosomesEmpty(previousFront)) {
                ...
                if(minimumRank != -1) for(Query chromosome : previousFront)
                    while(hasRecessiveRankGreaterThanZero(chromosome)) for(Query recessive : chromosome.getDominatedChromosomes()) {
     ...
     */
    public static List<ParetoObject> crowdingDistanceAssignment(List<Query> singularFront) {
         ...
         */
        for(Query chromosome : singularFront) 
            singlePareto.add(i++, new ParetoObject(chromosome, 0f));
     ...
     */
    public static boolean dominates_old(final Query competitor1, final Query competitor2) {
    ...
    public static boolean dominates(final Query competitor1, final Query competitor2) {
        
        if(competitor1.getPercision()>= competitor2.getPercision() && competitor1.getRecall()>competitor2.getRecall()){
            return true;
        }
        else{
            return false;
     ...
     */
    private static boolean isDominatedChromosomesEmpty(List<Query> front) {
     ...
     */
    private static boolean hasRecessiveRankGreaterThanZero(Query chromosome) {
    ...
    public static Population createCombinedPopulation(Population parent, Population child) {
        ...
        List<Query> combinedPopulace = new ArrayList<>();
        Population combinedPopulation = new Population();

        combinedPopulace.addAll(parent.getPopulation());
        combinedPopulace.addAll(child.getPopulation());
  ...
  /*  private static double minMaxNormalization(final double value) {
        return (((value - Configuration.ACTUAL_MIN) / (Configuration.ACTUAL_MAX - main.java.io.onclave.nsga.ii.configuration.Configuration.ACTUAL_MIN)) * (main.java.io.onclave.nsga.ii.configuration.Configuration.NORMALIZED_MAX - main.java.io.onclave.nsga.ii.configuration.Configuration.NORMALIZED_MIN)) + main.java.io.onclave.nsga.ii.configuration.Configuration.NORMALIZED_MIN;
    }*/
